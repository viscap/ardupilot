...
def drive_APMbalancer(viewerip=None, map=False):
    '''drive APMbalancer in SIL
    ...
    sil = util.start_SIL('APMbalancer', wipe=True)
    mavproxy = util.start_MAVProxy_SIL('APMbalancer', options=options, synthetic_clock=True)
    ...
    sil = util.start_SIL('APMbalancer')
    mavproxy = util.start_MAVProxy_SIL('APMbalancer', options=options, synthetic_clock=True)
    ...
    buildlog = util.reltopdir("../buildlogs/APMbalancer-test.tlog")
    ...
    try:
        ...
        if not drive_mission(mavproxy, mav, os.path.join(testdir, "rover1.txt")):
            ...
            failed = True
        if not log_download(mavproxy, mav, util.reltopdir("../buildlogs/APMbalancer-log.bin")):
...
#            failed = True
    ...
    if os.path.exists('APMbalancer-valgrind.log'):
        os.chmod('APMbalancer-valgrind.log', 0644)
        shutil.copy("APMbalancer-valgrind.log", util.reltopdir("../buildlogs/APMbalancer-valgrind.log"))
