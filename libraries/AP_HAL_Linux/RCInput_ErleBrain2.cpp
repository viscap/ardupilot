...
//Memory Addresses
#define RCIN_ERLEBRAIN2_RPI1_DMA_BASE 0x20007000
#define RCIN_ERLEBRAIN2_RPI1_CLK_BASE 0x20101000
#define RCIN_ERLEBRAIN2_RPI1_PCM_BASE 0x20203000

#define RCIN_ERLEBRAIN2_RPI2_DMA_BASE 0x3F007000
#define RCIN_ERLEBRAIN2_RPI2_CLK_BASE 0x3F101000
#define RCIN_ERLEBRAIN2_RPI2_PCM_BASE 0x3F203000

#define RCIN_ERLEBRAIN2_GPIO_LEV0_ADDR  0x7e200034
#define RCIN_ERLEBRAIN2_DMA_LEN         0x1000
#define RCIN_ERLEBRAIN2_CLK_LEN         0xA8
#define RCIN_ERLEBRAIN2_PCM_LEN         0x24
#define RCIN_ERLEBRAIN2_TIMER_BASE      0x7e003004

#define RCIN_ERLEBRAIN2_DMA_SRC_INC     (1<<8)
#define RCIN_ERLEBRAIN2_DMA_DEST_INC    (1<<4) 
#define RCIN_ERLEBRAIN2_DMA_NO_WIDE_BURSTS  (1<<26)
#define RCIN_ERLEBRAIN2_DMA_WAIT_RESP   (1<<3)
#define RCIN_ERLEBRAIN2_DMA_D_DREQ      (1<<6)
#define RCIN_ERLEBRAIN2_DMA_PER_MAP(x)  ((x)<<16)
#define RCIN_ERLEBRAIN2_DMA_END         (1<<1)
#define RCIN_ERLEBRAIN2_DMA_RESET       (1<<31)
#define RCIN_ERLEBRAIN2_DMA_INT         (1<<2)

#define RCIN_ERLEBRAIN2_DMA_CS          (0x00/4)
#define RCIN_ERLEBRAIN2_DMA_CONBLK_AD   (0x04/4)
#define RCIN_ERLEBRAIN2_DMA_DEBUG       (0x20/4)

#define RCIN_ERLEBRAIN2_PCM_CS_A        (0x00/4)
#define RCIN_ERLEBRAIN2_PCM_FIFO_A      (0x04/4)
#define RCIN_ERLEBRAIN2_PCM_MODE_A      (0x08/4)
#define RCIN_ERLEBRAIN2_PCM_RXC_A       (0x0c/4)
#define RCIN_ERLEBRAIN2_PCM_TXC_A       (0x10/4)
#define RCIN_ERLEBRAIN2_PCM_DREQ_A      (0x14/4)
#define RCIN_ERLEBRAIN2_PCM_INTEN_A     (0x18/4)
#define RCIN_ERLEBRAIN2_PCM_INT_STC_A   (0x1c/4)
#define RCIN_ERLEBRAIN2_PCM_GRAY        (0x20/4)

#define RCIN_ERLEBRAIN2_PCMCLK_CNTL     38
#define RCIN_ERLEBRAIN2_PCMCLK_DIV      39
...
void RCInput_ErleBrain2::set_physical_addresses(int version)
{
    if (version == 1) {
        ...
        clk_base = RCIN_ERLEBRAIN2_RPI1_CLK_BASE;
        pcm_base = RCIN_ERLEBRAIN2_RPI1_PCM_BASE;
    }                                                                                                                                     
    else if (version == 2) {
        dma_base = RCIN_ERLEBRAIN2_RPI2_DMA_BASE;
        clk_base = RCIN_ERLEBRAIN2_RPI2_CLK_BASE;
        pcm_base = RCIN_ERLEBRAIN2_RPI2_PCM_BASE;
    }
}
...
void RCInput_ErleBrain2::init_PCM()
{
    ...
    clk_reg[RCIN_ERLEBRAIN2_PCMCLK_CNTL] = 0x5A000006;                              // Source=PLLD (500MHz)
    hal.scheduler->delay_microseconds(100);
    clk_reg[RCIN_ERLEBRAIN2_PCMCLK_DIV] = 0x5A000000 | ((50000/RCIN_ERLEBRAIN2_SAMPLE_FREQ)<<12);   // Set pcm div. If we need to configure DMA frequency.
    hal.scheduler->delay_microseconds(100);
    clk_reg[RCIN_ERLEBRAIN2_PCMCLK_CNTL] = 0x5A000016;                              // Source=PLLD and enable
    hal.scheduler->delay_microseconds(100);
    pcm_reg[RCIN_ERLEBRAIN2_PCM_TXC_A] = 0<<31 | 1<<30 | 0<<20 | 0<<16;             // 1 channel, 8 bits
    hal.scheduler->delay_microseconds(100);
    pcm_reg[RCIN_ERLEBRAIN2_PCM_MODE_A] = (10 - 1) << 10;                           //PCM mode
    hal.scheduler->delay_microseconds(100);
    pcm_reg[RCIN_ERLEBRAIN2_PCM_CS_A] |= 1<<4 | 1<<3;                               // Clear FIFOs
    hal.scheduler->delay_microseconds(100);
    pcm_reg[RCIN_ERLEBRAIN2_PCM_DREQ_A] = 64<<24 | 64<<8;                           // DMA Req when one slot is free?
    hal.scheduler->delay_microseconds(100);
    pcm_reg[RCIN_ERLEBRAIN2_PCM_CS_A] |= 1<<9;                                      // Enable DMA
    hal.scheduler->delay_microseconds(100);
    pcm_reg[RCIN_ERLEBRAIN2_PCM_CS_A] |= 1<<2;                                      // Enable Tx
    hal.scheduler->delay_microseconds(100);
}
    ...
    state(RCIN_ERLEBRAIN2_INITIAL_STATE)
{
    ...
    circle_buffer = new Memory_table(RCIN_ERLEBRAIN2_BUFFER_LENGTH * 2, version);
...
void RCInput_ErleBrain2::init(void*)
{
    ...
    //Enable PPM input
    enable_pin = hal.gpio->channel(PPM_INPUT_ERLEBRAIN2);
...
void RCInput_ErleBrain2::_timer_tick()
{
    ...
    dma_cb_t* ad = (dma_cb_t*) con_blocks->get_virt_addr(dma_reg[RCIN_ERLEBRAIN2_DMA_CONBLK_AD | RCIN_ERLEBRAIN2_DMA_CHANNEL << 8]);
    for(j = 1; j >= -1; j--){
    ...
    for (;counter > 0x40;counter--) {
        ...
        if (curr_signal != last_signal) {
            ...
            prev_tick = curr_tick;
            switch (state) {
            case RCIN_ERLEBRAIN2_INITIAL_STATE:
                state = RCIN_ERLEBRAIN2_ZERO_STATE;
                break;
            case RCIN_ERLEBRAIN2_ZERO_STATE:
                if (curr_signal == 0) {
                    width_s0 = (uint16_t) delta_time;
                    state = RCIN_ERLEBRAIN2_ONE_STATE;
                    break;
                }
                else 
                    break;
            case RCIN_ERLEBRAIN2_ONE_STATE:
                if (curr_signal == 1) {
